...
key_distribution_plot = [0]*max(key_distribution) # before merge
entry_distribution_plot = [0]*max(entry_distribution) #after merge

for key in key_distribution:
  key_distribution_plot[key-1] += 1
for key in entry_distribution:
  entry_distribution_plot[key-1] += 1
with open(datafile, 'w') as the_data_file:
  for i in range(len(key_distribution_plot)):
    the_data_file.write(str(i+1)+'\t'+str(key_distribution_plot[i])+'\n')

with open(mergefile, 'w') as the_data_file:
  for i in range(len(entry_distribution_plot)):
    the_data_file.write(str(i+1)+'\t'+str(entry_distribution_plot[i])+'\n')
